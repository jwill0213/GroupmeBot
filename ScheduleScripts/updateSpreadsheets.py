...
PERCENT_FORMAT = CellFormat(numberFormat=NumberFormat(
    type='PERCENT', pattern='0.000%'))

GREEN_COLOR_FORMAT = CellFormat(
    backgroundColor=Color(red=0.7176471,
                          green=0.88235295,
                          blue=0.8039216),
    backgroundColorStyle=ColorStyle(rgbColor=Color(red=0.7176471,
                                    green=0.88235295,
                                    blue=0.8039216)))

GREATER_THAN_RULE = BooleanRule(condition=BooleanCondition(
    'NUMBER_GREATER_THAN_EQ', ['0']), format=GREEN_COLOR_FORMAT)

RED_COLOR_FORMAT = CellFormat(
    backgroundColor=Color(red=0.95686275,
                          green=0.78039217,
                          blue=0.7647059),
    backgroundColorStyle=ColorStyle(rgbColor=Color(red=0.95686275,
                                    green=0.78039217,
                                    blue=0.7647059)))

LESS_THAN_RULE = BooleanRule(condition=BooleanCondition(
    'NUMBER_LESS', ['0']), format=RED_COLOR_FORMAT)
...
def main():
    ...
    for botID in bots:
        ...
        dataDatetime = datetime.now().astimezone(
            timezone("US/Central")).strftime("%Y-%m-%d %I:%M:%S %p")
        ...
        for symbol, quote in priceData['quotes'].items():
            if symbol in [w.title for w in spreadsheet.worksheets()]:
                ...
                sheet = spreadsheet.duplicate_sheet(
                    source_sheet_id=template.id, new_sheet_name=symbol, insert_sheet_index=99)

            startCell = sheet.find('Datetime')

            # Record price
            sheetUpdate = sheet.append_row([dataDatetime, quote['currentPrice']], value_input_option=ValueInputOption.user_entered,
                                           table_range=startCell.address)

            # Find the cells to update with percent change
            dateCell, priceCell = sheetUpdate['updates']['updatedRange'].split('!')[
                1].split(':')
            ...
            if updatedRow > 2:
                priceChangeCell = sheet.find(
                    'Price Change').address[0] + str(updatedRow)
                percentChangeCell = sheet.find(
                    'Percent Change').address[0] + str(updatedRow)
                ...
                formattingRules.append(ConditionalFormatRule(
            ...
            cellAddress = athHeaderCell.address[0] + \
                str(int(athHeaderCell.address[1:]) + 1)
            ...
            if athData:
                ...
                if athCell.value:
                    ...
                    logging.debug(
                        f'No All Time High data found. Updating to {newATH}')
                    sheet.update_acell(cellAddress, newATH)
