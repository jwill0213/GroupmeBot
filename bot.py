...
for group in BOT_INFO:
    try:
        ...
        logging.debug(
            f"Group rules ({groupAlias}) not found for [{botName}:{botID}].",
            exc_info=True,
        )
...
def logmsg(data):
    try:
        ...
        logging.warning("Message data does not contain a sender_type.", exc_info=True)
    else:
        if sender_type == "user":
            logging.info(
...
def webhook():
    ...
    try:
        if groupID in GROUP_RULES and GROUP_RULES[groupID].run(
            data, botID, send_message
        ):
