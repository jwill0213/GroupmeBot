...
def attach_type(attachments):
    types = {"image": "[IMG] ", "location": "[LOC] ", "poll": "", "event": ""}
    typelist = ""
    for attachment in attachments:
        try:
            typelist += types[attachment["type"]]
        except KeyError:
            logging.warning("Attachment type {} unknown.".format(attachment["type"]))
    return typelist
...
def send_message(msg, bot_id, mentions=None):
    ...
    if mentions:
        data["attachments"] = [{"loci": [], "type": "mentions", "user_ids": []}]
        for mention in mentions:
            startIndex = len(msg) - 1
            data["text"] = data["text"] + mention["tag"]
            data["attachments"][0]["loci"].append([startIndex, len(mention["tag"])])
